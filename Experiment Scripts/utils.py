...
def Assign_prob_trigram(trigram, trie, alpha = 0.4):
...
#   using stupid backoff
    ...
    if x:
        # if bigram-prefix exists and trigram exists
    ...
    # print(f'trigram {trigram} does not exist but its bigram-prefix {trigram[:2]} does: ')
        y = in_trie(trigram[1:],trie,[])
        if y:
            # if leftover-bigram exists but trigram does not exist
            self_count = trie[y[0]][y[1]][count_sign]
            parent_count =  trie[y[0]][count_sign]
...
#             return np.log(C2)-np.log(parent_count)-np.log(vocab-parent_count)
        else:
            return np.log(1/vocab)+2*np.log(alpha)
                #if bigram-pre does not, unigram-pre de leftover-bigram exist and leftover-bigram exist:
                #print('bigram backoff:', trigram[1]+' '+trigram[2])
#                 self_count = trie[z[0]][z[1]][count_sign]
#                 parent_count =  trie[z[0]][count_sign]
#                 return np.log(self_count)-np.log(parent_count)+np.log(alpha)
#                 zz = in_trie(trigram[1],trie,[])
#                 if zz:
#                    # if bigram-pre exist not, unigram-pre de l-bigram exists but l-bigram not.
...
# #                     GET THE PARENT COUNT FOR ESTIMATION
#                     if C1==0:
#                         C1=1
#                     parent_count = trie[zz[0]][count_sign] 
#                     return np.log(C1)-np.log(parent_count)-np.log(vocab-parent_count)+np.log(alpha)
#                 else:
#                      #print('last word is ', trigram[2])
#                     zzz = in_trie(trigram[2],trie,[])
#                     if zzz:
#                        # if the uni-pre d l-bigram not exist
...
#                     else:
#                         #if bi-pre and uni-pre de l-bi exist not, and last word is unk
#                         print(f'{trigram[1]} and {trigram[2]} are 2 consec unks in {trigram}')
#                         #   Good turing
#                         C0 = 1
#                         for key in trie.keys():
#                             if key!=count_sign:
...
#                         return np.log(C0)-np.log(parent_count)-np.log(vocab-parent_count) + np.log(alpha)*2

def AssignProbSent(sentence, trie, count_sign = '__COUNT__'): 
...
#     ax.tick_params(axis='y', colors='white')
    ...
    ay.set_xlabel('tokens',fontsize=14)
    ay.set_ylabel('log probability')
